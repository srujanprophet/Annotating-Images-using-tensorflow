...
try:
	...
	# If FFmpeg is not found , download from the internet
	import imageio
...
class DetectionObj(object):
	"""
	DetectionObj is a class suitable to leverage Google
	Tensorflow detection API for image annotation from 
	different sources: files, images acquired by own's
	webcam, videos.
	"""

	def __init__(self, model='ssd_mobilenet_v1_coco_11_06_2017'):
		"""
		The instruction to be run when the class is instantiated
		"""
		...
		# Attempting loading the detection model, if not
		# available, download from internet.
		try:
		...
		extracted_categories = label_map_util.convert_label_map_to_categories(
	...
	def download_frozen_model(self):
		...
		# Downloading the model with tqdm estomations of completion
		print('Downloading...')
		with tqdm() as t:
	...
	def detect(self, images, annotate_on_image=True):
		...
		for image in images:
			...
			# Each score represent level of confidence
			...
			if annotate_on_image:
				...
				results.append((new_image, boxes, scores, 
	...
	def detection_on_image(self, image_np, boxes, scores,
							classes):
		"""
		Put detection boxes on the image over
	...
	def annotate_photogram(self, photogram):
		"""
		Annotating a video's photogram with bounding 
		boxes over detected classes
		"""
	...
	def capture_webcam(self):
		...
		# adjusts to the surrounding loghts
		ramp_frames = 30
		...
		# discarded as the camera adjusts to the right light levels
	...
	def webcam_pipeline(self):
		...
		saving_path = os.path.join(self.CURRENT_PATH, "webcam/" + filename + ".jpg")
